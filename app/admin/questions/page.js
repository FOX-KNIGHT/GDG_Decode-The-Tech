...
const ROUND_LABELS = { 1: 'Tech Word Decoder', 2: 'Guess The Tech', 3: 'Real or Fake?' };
const ROUND_COLORS = { 1: 'gdg-blue', 2: 'gdg-yellow', 3: 'gdg-red' };
const EMPTY_FORM = { round: 1, question: '', emojiClue: '', options: ['', '', '', ''], correctAnswer: '', actualFact: 'Real', explanation: '', basePoints: 10 };
...
export default function AdminQuestionsPage() {
  ...
  function startEdit(q) {
    setEditing(q._id);
    setForm({ round: q.round, question: q.question, emojiClue: q.emojiClue || '', options: q.options.concat(['','','','']).slice(0,4), correctAnswer: q.correctAnswer, actualFact: q.actualFact || 'Real', explanation: q.explanation || '', basePoints: q.basePoints || 10 });
    setShowForm(true);
  ...
  async function saveQuestion(e) {
    ...
    const payload = { ...form, round: parseInt(form.round), basePoints: parseInt(form.basePoints), options: form.options.filter(Boolean) };
    if (!payload.options.includes(payload.correctAnswer)) { showMsg('Correct answer must match one of the options exactly!', 'error'); setSaving(false); return; }
    try {
      const method = editing ? 'PUT' : 'POST';
      const body = editing ? { _id: editing, ...payload } : payload;
      const res = await fetch('/api/questions', { method, headers: { 'Content-Type': 'application/json', 'x-admin-password': adminPass }, body: JSON.stringify(body) });
      if (res.ok) { showMsg(editing ? 'Question updated!' : 'Question added!'); resetForm(); fetchQuestions(); }
            ...
            <form onSubmit={saveQuestion} className="space-y-6 relative z-10">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  ...
                  <select value={form.round} onChange={e => setForm({ ...form, round: parseInt(e.target.value) })}
                    ...
                    <option value={1}>PHASE 01 — Tech Word Decoder</option>
              ...
              <div>
                <label className="font-mono text-[9px] text-gdg-blue tracking-[0.3em] block mb-2 uppercase">
                  {form.round === 1 ? 'QUERY_TEXT — Write the MEANING/DEFINITION *' : form.round === 2 ? 'QUERY_TEXT *' : 'TECH FACT STATEMENT *'}
                </label>
                <textarea required value={form.question} onChange={e => setForm({ ...form, question: e.target.value })} rows={3}
                  placeholder={form.round === 1 ? 'e.g. Online storage and computing accessed via the internet' : form.round === 2 ? 'Identify the app or platform from this emoji clue' : 'e.g. Google tested smart contact lenses that could monitor blood sugar'}
              ...
              <div className="bg-dark-950/50 p-6 border border-white/5 clip-angled">
                <label className="font-mono text-[9px] text-gdg-blue tracking-[0.3em] block mb-4 uppercase">
                  {form.round === 1 ? 'AVAILABLE_VECTORS (Jargons) *' : form.round === 2 ? 'AVAILABLE_VECTORS (App names) *' : 'AVAILABLE_VECTORS ("Real" / "Fake") *'}
                </label>
                  ...
                  {form.options.map((opt, i) => (
                    <div key={i} className="flex items-stretch clip-angled-br">
                      ...
                      <input value={opt} onChange={e => updateOption(i, e.target.value)}
                        placeholder={form.round === 3 ? (i === 0 ? 'Real' : i === 1 ? 'Fake' : '') : `Option ${['A','B','C','D'][i]}`}
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-dark-950/50 p-6 border border-white/5 clip-angled">
                <div>
                  <label className="font-mono text-[9px] text-gdg-green tracking-[0.3em] block mb-2 uppercase">
                    VALID_SIGNATURE (Correct Answer) *{form.round === 3 ? ' [MUST BE OPPOSITE FACT]' : ''}
                  </label>
                ...
                {form.round === 3 && (
                  <div>
                    <label className="font-mono text-[9px] text-gray-400 tracking-[0.3em] block mb-2 uppercase">ACTUAL_STATE (For records)</label>
              ...
              <div>
                ...
                <input value={form.explanation} onChange={e => setForm({ ...form, explanation: e.target.value })}
                  ...
                  className="w-full bg-dark-950 border border-white/10 text-white font-body text-sm px-5 py-4 focus:outline-none focus:border-gdg-blue placeholder-white/20 transition-colors" />
              </div>
        ...
        <div className="space-y-4 pt-4">
          ...
          {loading ? (
            ...
            <div className="text-center py-24 border border-white/5 border-dashed bg-dark-900/50 clip-angled">
              ...
              <div className="font-mono text-[9px] text-gray-600 tracking-widest uppercase">Run SEED_DATABASE or commit manually.</div>
            </div>
          ) : filtered.map((q, idx) => (
            <div key={q._id} className={`clip-angled tech-border p-6 hover:-translate-x-2 transition-all relative group bg-dark-900/80 hover:bg-dark-900 border-l-[3px] border-l-${ROUND_COLORS[q.round]}`}>
              ...
              <div className="flex flex-col md:flex-row items-start gap-6">
                ...
                <div className="flex flex-row md:flex-col gap-2 flex-shrink-0 w-full md:w-auto justify-end md:justify-start border-t md:border-t-0 border-white/10 pt-4 md:pt-0 relative z-10">
                  <div className="font-mono text-[9px] text-gdg-blue text-center px-4 py-2 bg-dark-950 flex-1 md:flex-none border border-white/5 uppercase tracking-[0.3em] clip-hex">
                    {q.basePoints} CYC
                  </div>
